...
class Decision_Tree:
	...
	def Heuristic(self, train_values,Class,Attributes):
		...
		if Entropy==0 or len(Attributes)==0:
			...
			Attr=self.Entropyy(train_values,Class,Attributes,Entropy)
			if Attr==-1:
				return Rnode
	...
	def Entropyy(self,train_values,Class,Attributes,Entropy):


		Attr,Maxgain=-1,-1

		for attribute in Attributes:
				tree_gain=self.EntropyGain(train_values,Class,Entropy,attribute)

				if tree_gain > Maxgain:
					Maxgain=tree_gain
					Attr=attribute
		return Attr
	...
	def FindVariance(self,train_values,Class):
		...
		return 	ar*eksi
	def EntropyGain(self,train_values,Class,Entropy,attribute):

		trEntropy=len(train_values)


		temp_tree=self.Divide(train_values,Class,attribute)
		
		Entropy1=self.EntropyCalc(temp_tree[0][0],temp_tree[0][1])
		Entropy2=self.EntropyCalc(temp_tree[1][0],temp_tree[1][1])

		x=1.0*len(temp_tree[0][0])/trEntropy
		y=1-x

		infoGain = Entropy -x* Entropy1 -y * Entropy2 
		return infoGain
